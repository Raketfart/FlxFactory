...
class FlipButton extends FlxSprite
{
	...
	public var onTurnon(default, null):FlxButtonEvent;

	public var onTurnoff(default, null):FlxButtonEvent;
	...
	public function new(X:Float = 0, Y:Float = 0, graphics:Dynamic, ?OnClickOn:Void->Void, ?OnClickOff:Void->Void,  isFlipped:Bool = false)
	{
		...
		loadGraphic(graphics, true, 21, 21);
		
		onUp = new FlxButtonEvent();
		onDown = new FlxButtonEvent();
		onOver = new FlxButtonEvent();
		onOut = new FlxButtonEvent();
		onTurnon = new FlxButtonEvent(OnClickOn);
		onTurnoff = new FlxButtonEvent(OnClickOff);
	...
	override public function destroy():Void
	{
		...
		onTurnon = FlxDestroyUtil.destroy(onTurnon);
		onTurnoff = FlxDestroyUtil.destroy(onTurnoff);
	...
	private function onUpHandler():Void
	{
		...
		if (statusFlip == FlxButton.NORMAL)
		{
			statusFlip = FlxButton.PRESSED;
